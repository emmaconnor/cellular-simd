...
void do_ca(uint8_t *grid, uint8_t *rules, int width, int ngens) {
    /* we're going to sentinalize the front and back of the list to simulate
       continuous boundary conditions, so add space for 2 more items */
    ...
    __m128i *new, *old, *end;
    __m128i table = _mm_loadu_si128((__m128i*)rules);
    __m128i tmp;
